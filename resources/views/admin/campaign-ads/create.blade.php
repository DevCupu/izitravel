<x-admin-layout :title="__('Tambah Ads')">
    <x-slot name="header">
        <div class="flex items-center gap-2">
            ...
            <div><h2 class="text-lg font-bold text-slate-900 dark:text-white">Tambah Ads</h2><p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Buat link dan pesan khusus untuk satu materi iklan.</p></div>
        </div>
    </x-slot>
    <div class="max-w-3xl animate-fade-in-up">
        <div class="content-card bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 p-6 sm:p-8">
            <form method="POST" action="{{ route('admin.campaigns.ads.store', $campaign) }}" class="space-y-6">
                @csrf
                @include('admin.campaign-ads._form')
                <div class="flex items-center gap-3 pt-4 border-t border-slate-100 dark:border-slate-700">
                    <button type="submit" class="inline-flex items-center gap-2 px-6 py-2.5 bg-blue-600 hover:bg-blue-700 rounded-xl font-bold text-sm text-white transition"><i data-lucide="save" class="w-4 h-4"></i>Simpan Ads</button>
