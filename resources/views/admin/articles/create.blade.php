<x-admin-layout :title="__('Tambah Artikel')">
    <x-slot name="header">
        <div class="flex items-center gap-2">
            <a href="{{ route('admin.articles.index') }}" class="p-1.5 rounded-lg text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-700 transition">
            ...
            <div>
                ...
                <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5 hidden sm:block">
                    {{ __('Tambahkan postingan artikel panduan baru ke landing page.') }}
                </p>
            </div>
        </div>
    </x-slot>
        ...
        <div class="content-card bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 p-6 sm:p-8">
            <form method="POST" action="{{ route('admin.articles.store') }}" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="flex items-center gap-3 pt-4 border-t border-slate-100 dark:border-slate-700">
                    ...
                    <a href="{{ route('admin.articles.index') }}" class="inline-flex items-center justify-center px-6 py-2.5 rounded-xl font-bold text-sm text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-700 transition">
                        {{ __('Batal') }}
                    </a>
                </div>
            </form>
