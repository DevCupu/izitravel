<x-admin-layout :title="__('Ubah Kategori')">
    <x-slot name="header">
        <div class="flex items-center gap-2">
            ...
            <div>
                <h2 class="text-lg font-bold text-slate-900 dark:text-white leading-tight">{{ __('Ubah Kategori') }}</h2>
                <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5 hidden sm:block">{{ __('Perbarui kategori :name.', ['name' => $category->name]) }}</p>
            </div>
        </div>
    </x-slot>
