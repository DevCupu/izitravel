<x-admin-layout :title="__('Ubah Testimoni')">
    <x-slot name="header">
        <div class="flex items-center gap-2">
            ...
            <div>
                <h2 class="text-lg font-bold text-slate-900 dark:text-white leading-tight">{{ __('Ubah Testimoni') }}</h2>
                <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5 hidden sm:block">{{ __('Perbarui testimoni dari :name.', ['name' => $testimonial->name]) }}</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="max-w-4xl animate-fade-in-up">
        <div class="content-card bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 p-6 sm:p-8">
            <form method="POST" action="{{ route('admin.testimonials.update', $testimonial) }}" class="space-y-6" enctype="multipart/form-data">
                @csrf
                @method('PUT')
