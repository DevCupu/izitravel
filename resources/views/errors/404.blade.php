...
<head>
    ...
    <title>404 - Halaman Tidak Ditemukan | IZI Travel</title>
    
    <!-- Google Fonts: Outfit, Inter, El Messiri -->
    ...
    <!-- Tailwind CSS via CDN for robust, standalone error page styling -->
...
<body class="bg-gradient-to-br from-slate-950 via-indigo-950 to-slate-900 text-slate-100 min-h-screen flex items-center justify-center relative overflow-hidden font-sans antialiased">
    <!-- Starry Islamic Geometric Pattern Overlay -->
    ...
    <div class="absolute -right-32 -top-32 w-[600px] h-[600px] bg-blue-600/10 rounded-full blur-[130px] pointer-events-none"></div>
    <div class="absolute -left-32 -bottom-32 w-[600px] h-[600px] bg-amber-500/10 rounded-full blur-[130px] pointer-events-none"></div>
    ...
    <div class="relative z-10 max-w-lg mx-4 text-center bg-white/5 backdrop-blur-xl border border-white/10 p-8 sm:p-16 rounded-[2.5rem] shadow-2xl flex flex-col items-center">
        <!-- Crescent Moon & Star Illustration -->
        <div class="animate-bounce-slow mb-6">
            <svg class="w-20 h-20 text-amber-400 fill-current fill-opacity-10 drop-shadow-[0_4px_12px_rgba(245,158,11,0.3)]" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.2">
                <!-- Crescent -->
                <path d="M12 3a9 9 0 1 0 9 9 9.75 9.75 0 0 1-9-9Z" stroke-linejoin="round"/>
                <!-- Star -->
                <path d="M16.5 7.5l.4 1.1 1.1.4-1.1.4-.4 1.1-.4-1.1-1.1-.4 1.1-.4z" stroke-width="0.5"/>
            </svg>
        </div>

        <!-- Huge 404 Text -->
        <h1 class="text-8xl sm:text-9xl font-black tracking-widest text-transparent bg-clip-text bg-gradient-to-r from-amber-400 via-yellow-300 to-amber-500 animate-pulse-slow font-heading">
            404
        </h1>

        <!-- Error Heading -->
        <h2 class="text-2xl sm:text-3xl font-extrabold text-white mt-6 mb-3 font-heading tracking-tight">
            Halaman Tidak Ditemukan
        </h2>

        <!-- Error Description -->
        <p class="text-slate-400 text-xs sm:text-sm leading-relaxed mb-8 max-w-sm">
            Mohon maaf, halaman yang Anda cari tidak dapat ditemukan. Mungkin tautan telah dipindahkan, dihapus, atau alamat URL salah diketik.
        </p>
        ...
        <!-- Small Branding -->
        <p class="text-[10px] sm:text-xs text-slate-500 mt-8 font-bold uppercase tracking-widest font-heading">
            IZI Travel
        </p>
    </div>
</body>
